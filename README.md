...
| knowledge-only 방식으로 전환해줘. | 용도에 맞게 운영 형태 전환 |
| OKF 버전을 올려줘. | 공식 명세 변경의 영향과 전환 필요성 판단 |
| 현재 OKF를 점검해줘. | 현재 구성과 지식의 적절성 진단 |
| 현재 OKF 구조를 점검하고 필요한 부분을 고쳐줘. | 진단과 필요한 개선 반영 |

표현을 정확히 맞추거나 내부 작업 이름을 알 필요는 없습니다. 에이전트는 [적용 안내](APPLICATION.md)에서 시작해 요청과 대상의 현재 상태를 함께 해석합니다. “적용해줘”는 필요한 작업을 종합 판단하는 요청이며, 구체적인 요청에서는 그 목적과 범위에 맞게 관련 철학과 프로필을 탐색합니다.
...
채택한 판단 근거는 `okf/`에 두고, `profiles/`와 `templates/`에 적용 가능한 형태로 연결합니다. `notes/`와 `experiments/`는 연구 자료이며 미채택 내용은 기본 적용 근거로 삼지 않습니다. 실제 운영에서 얻은 관찰은 이곳에 남기고, 재사용할 만한 결론을 관련 개념과 적용 자료에 반영합니다.
